<div class="footer">
		<div class="ft">
			<div style="position:relative;overflow:hidden;">
				<iframe
					src="https://yandex.ru/map-widget/v1/?ll=56.008443%2C54.716535&mode=search&oid=1157680995&ol=biz&sctx=ZAAAAAgBEAAaKAoSCZj2zf3V%2F0tAEXAjZYukW0tAEhIJPu3w12SNmj8RcOoDyTuHgj8iBgABAgMEBSgKOABAlc0GSABqAnJ1nQHNzEw9oAEAqAEAvQFo36RgwgEF456DqATqAQDyAQD4AQCCAhzRhNC40YLQvdC10YEg0LDRgNC10L3QsCAzMDAwigIAkgIAmgIMZGVza3RvcC1tYXBz&sll=56.008443%2C54.716535&sspn=0.015525%2C0.005417&text=%D1%84%D0%B8%D1%82%D0%BD%D0%B5%20%D0%B0%D1%80%D0%B5%D0%BD%D0%B0%203000&z=16.54"
					width="500px" height="300px" style="position:relative; border: none; margin-right:30px;">
				</iframe>
			</div>
			<div class="cont">
				<h1>Наши контакты</h1>
				+7 (347) 216-3000<br>
				<a href="">г. Уфа, ул. Менделеева, 134/7</a><br>
				<p>ПН-ПТ 7:00-00:00</p>
				<p>ПН-ПТ 7:00-22:00</p>
				<a href="">parena@fitness3000.ru</a><br>
				<a href="https://vk.com/fitnessarena3000"><img alt="" src="Resources/c1.png"></a>
				<a href="https://api.whatsapp.com/send?phone=79177400625"><img alt="" src="Resources/c2.png"></a>
			</div>
		</div>
		<div class="ft2">
			<span id="span_1"><a
					href="https://fitness3000.ru/upload/29.05.2023.%D0%9E%D0%B1%D1%89%D0%B8%D0%B5%20%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0.pdf">Правила
					клуба</a></span>
			<span style="float:right;"><a href="">Политика конфиденциальности</a></span>
		</div>
		<script src="3000_script.js"></script>

</div>